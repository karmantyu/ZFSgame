...
our $ZFS_PROFILEPROPERTY_PROP = 'org.zfsguru:profileproperty';
...
sub _sanitize_acl_user_prop_tokens
{
    my ($val) = @_;
    my @raw = split(/[\0,\s]+/, ($val || ''));
    my %seen;
...
sub validate_posix_mode
{
    ...
    $mode =~ s/^\s+|\s+$//g;
    $mode =~ s/^0+// if (length($mode) > 4);
    return ($mode =~ /^[0-7]{3,4}$/) ? 1 : 0;
}

sub normalize_posix_mode_input
{
    my ($mode) = @_;
    return '' if (!defined $mode);
    $mode =~ s/^\s+|\s+$//g;
    $mode =~ s/^0+// if (length($mode) > 4);
    return validate_posix_mode($mode) ? $mode : '';
}
...
sub parse_acl_users_prop
{
    ...
    return (join("\n", @users), $jail_uid, $jail_gid);
}

sub build_acl_users_prop_value
{
    my ($users_nl, $jail_uid, $jail_gid) = @_;
    my @tokens = _sanitize_user_list($users_nl);
    if (defined($jail_uid) && $jail_uid =~ /^[0-9]+$/) {
        push @tokens, 'JUID'.$jail_uid;
    }
    if (defined($jail_gid) && $jail_gid =~ /^[0-9]+$/) {
        push @tokens, 'JGID'.$jail_gid;
    }
    return join(' ', @tokens);
}
...
{
    my ($user) = @_;
    return $1 if (defined($user) && $user =~ /^JUID([0-9]+)$/);
    return undef if (defined($user) && $user =~ /^JGID[0-9]+$/);
    return $user;
}
...
sub set_acl_users_for_dataset
{
    my ($ds, $val) = @_;
    return 0 if (!$ds);
    my @users = _sanitize_acl_user_prop_tokens($val);
    if (!@users) {
...
sub get_profileproperty_for_dataset
{
    my ($ds) = @_;
    return '' if (!$ds);
    my $val = _run_cmd("zfs get -H -o value $ZFS_PROFILEPROPERTY_PROP "._quote_shell($ds));
    return '' if (!$val || $val eq '-');
    return $val;
}

sub parse_profileproperty_modes
{
    my ($val) = @_;
    return ('', '') if (!defined($val) || $val eq '');
    foreach my $token (split(/[\0,\s]+/, $val)) {
        next if (!$token);
        if ($token =~ /^O([0-7]{3})([0-7]{3})$/) {
            return ($1, $2);
        }
        if ($token =~ /^O([0-7]{3})([0-7]{4})$/) {
            return ($1, $2);
        }
        if ($token =~ /^O([0-7]{4})([0-7]{4})$/) {
            return ($1, $2);
        }
    }
    return ('', '');
...
sub set_profileproperty_modes_for_dataset
{
    my ($ds, $custom_file, $custom_dir, $profile) = @_;
    return 0 if (!$ds);
    $custom_file = normalize_posix_mode_input($custom_file);
    $custom_dir = normalize_posix_mode_input($custom_dir);
    if ($custom_file eq '' && $custom_dir eq '') {
        _run_cmd("zfs inherit "._quote_shell($ZFS_PROFILEPROPERTY_PROP)." "._quote_shell($ds));
        return 1;
    }
    my ($default_dir, $default_file) = profile_modes($profile || 'MEDIA');
    $custom_file = $default_file if ($custom_file eq '');
    $custom_dir = $default_dir if ($custom_dir eq '');
    my $prop = $ZFS_PROFILEPROPERTY_PROP."=O".$custom_file.$custom_dir;
    _run_cmd("zfs set "._quote_shell($prop)." "._quote_shell($ds));
    return 1;
}
...
sub profile_modes
{
    ...
    $file = $custom_file if ($custom_file ne '');
    $dir = $custom_dir if ($custom_dir ne '');
    return ($dir, $file);
}
...
sub detect_target_info
{
    ...
    if ($info{dataset}) {
        ...
        $info{profileproperty} = get_profileproperty_for_dataset($info{dataset});
        my ($users_nl, $jail_uid, $jail_gid) = parse_acl_users_prop($info{acl_users});
        $info{acl_users} = $users_nl;
        $info{jail_uid} = $jail_uid;
        $info{jail_gid} = $jail_gid;
        my ($custom_file, $custom_dir) = parse_profileproperty_modes($info{profileproperty});
        $info{custom_mode_file} = $custom_file;
        $info{custom_mode_dir} = $custom_dir;
    }

    my ($mode_dir, $mode_file) = profile_modes($info{profile}, $info{custom_mode_file}, $info{custom_mode_dir});
...
sub add_users_acl
{
    ...
    foreach my $u (split(/\s+/, $users || "")) {
        ...
        next if (user_exists_in_acl($acltxt, $norm));
        my $ace;
        if ($isdir) {
...
sub posix_reset
{
    ...
    if (!$MODE_DIR || !$MODE_FILE) {
        ...
        elsif ($PROFILE eq "EXEC") { $MODE_DIR ||= "755"; $MODE_FILE ||= "755"; }
...
sub load_policy_users
{
    ...
    if ($DATASET) {
        ...
        if ($val) {
            my @users;
            foreach my $u (split(/\s+/, $val)) {
                next if ($u eq '');
                my $norm = normalize_acl_user_token($u);
                push @users, $norm if (defined($norm) && $norm ne '');
            }
...
sub run_acl_manager
{
    ...
    apply_profile();
    my $custom_file = normalize_posix_mode_input(
        defined($opt{custom_mode_file}) ? $opt{custom_mode_file} : $info->{custom_mode_file}
    );
    my $custom_dir = normalize_posix_mode_input(
        defined($opt{custom_mode_dir}) ? $opt{custom_mode_dir} : $info->{custom_mode_dir}
    );
    $MODE_FILE = $custom_file if ($custom_file ne '');
    $MODE_DIR = $custom_dir if ($custom_dir ne '');
    ...
    if ($opt{base_owner} && $opt{base_group}) {
        ...
        my $jail_uid = (defined($opt{jail_uid}) && $opt{jail_uid} ne '') ? $opt{jail_uid} : $info->{jail_uid};
        my $jail_gid = (defined($opt{jail_gid}) && $opt{jail_gid} ne '') ? $opt{jail_gid} : $info->{jail_gid};
        if (defined($jail_uid) && $jail_uid =~ /^[0-9]+$/ && defined($jail_gid) && $jail_gid =~ /^[0-9]+$/) {
            $POSIX_UID = $jail_uid;
            $POSIX_GID = $jail_gid;
        }
    }
