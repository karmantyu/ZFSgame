...
sub _build_run_context_html
{
    ...
    my $html = &ui_table_start($text{'aclm_run_context'} || 'Run context', undef, 2);
    $html .= &ui_table_row($text{'aclm_mode'}, &html_escape($mode_label));
    $html .= &ui_table_row($text{'aclm_target'}, &html_escape($target));
    $html .= &ui_table_row($text{'aclm_detected_type'}, &html_escape($info->{type}));
    $html .= &ui_table_row($text{'aclm_dataset'},
        $info->{dataset} ? &html_escape($info->{dataset}) : 'N/A');
    $html .= &ui_table_row($text{'aclm_mountpoint'},
    ...
    if ($acl_lines && @$acl_lines) {
        ...
        $html .= &ui_table_row($text{'aclm_acl_base_lines'} || 'ACL base lines',
...
&ui_print_header(undef, $text{'aclm_title'}, "");

if ($target eq '') {
    print &ui_table_start($text{'aclm_errors'}, undef, 1);
    print &ui_table_row($text{'aclm_errors'}, &html_escape($text{'aclm_err_target'}));
    print &ui_table_end();
    &ui_print_footer('zfsaclmanager.cgi', $text{'aclm_title'});
    exit;
}
...
if (!defined $pid) {
    ...
    my $out = join("\n", @$log);
    print &ui_table_start($text{'aclm_output'}, undef, 1);
    print &ui_table_row('',
    ...
    print "<script>\n".
          ...
          "      navigator.sendBeacon('zfsaclmanager_apply.cgi', blob);\n".
          "      return;\n".
          "    } catch(e) {}\n".
          "  }\n".
          "  if (window.fetch) {\n".
          "    try {\n".
          "      fetch('zfsaclmanager_apply.cgi', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body: params, keepalive: true});\n".
          "      return;\n".
          "    } catch(e) {}\n".
          "  }\n".
          "  try {\n".
          "    var xhr = new XMLHttpRequest();\n".
          "    xhr.open('POST', 'zfsaclmanager_apply.cgi', false);\n".
...
print &ui_form_start('zfsaclmanager.cgi', 'get', undef, $back_tag);
